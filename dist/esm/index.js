...
var moduleFilename = typeof __filename === "string" ? __filename : fileURLToPath(import.meta.url);
var moduleDirname = typeof __dirname === "string" ? __dirname : path.dirname(moduleFilename);
...
function loadRawConfig(configPath = path.join(moduleDirname, "models.json")) {
...
function loadModelsConfiguration(options = {}) {
  const configPath = options.configPath || process.env.LLM_MODELS_CONFIG_PATH || path.join(__dirname, "models.json");
...
async function resolveModuleExports(moduleId, baseDir) {
  ...
  const resolvedId = isRelativeOrAbsolute ? path2.resolve(baseDir, moduleId) : moduleId;
  if (isRelativeOrAbsolute) {
    const moduleUrl = pathToFileURL(resolvedId).href;
    const exports2 = await import(moduleUrl);
...
async function registerProvidersFromConfig(modelsConfiguration3, options = {}) {
  const warnings = [];
  const baseDir = options.baseDir || (modelsConfiguration3.path ? path2.dirname(modelsConfiguration3.path) : __dirname);
...
export {
  ...
  registerDefaultLLMAgent2 as registerDefaultLLMAgent,
  registerLLMAgent,
  registerOperator
};
